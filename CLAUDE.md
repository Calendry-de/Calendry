...
| Person absences | ONE row per absence is the unit of approval, and the row is NOT the wire message: `absent_from`/`absent_to` carry the dates, `days/blocks/weeks` keep the whole touched weeks for listing and counting, and `approvedBlackoutsFor` is the ONLY place a dated row is expanded (via `resolveHolidayWeeks(...).windows`) into day-precise windows. Never send a dated row's stored triple; never split an absence into rows. | § "A mid-week absence blocks the days away" |
...
| Shared footprint | `room_footprint` is a symmetric PAIR relation (mirrored by trigger), edited from EITHER room as `rooms/footprint`; the wire's `footprint_tags` are DERIVED, one per pair, never stored. The hall pairs with each part; parts pair only if paired. Virtual rooms refused at the write and by trigger. | § "Shared footprints" |
...
| TimeGrid breaks | Reach the solver ONLY as `TimeGrid.default_gap_minutes`/`breaks`, read by ONE evaluator (`minimize_break_spanning`, issue #26); adjacency and conflict stay block-indexed. A multi-block Session spanning a break is still LEGAL and drawn honestly; the solver now prices it, never refuses it. `blockTime()`/`blockOfMinute()`/`gapsWithinSpan()` are the single definition of block boundaries and of what a span does not teach, mirrored by the solver's `gap_minutes_within_span`. | §§ "TimeGrid breaks", "A Session that spans a break" |
