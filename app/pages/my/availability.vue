 ...
 * leaves EXACTLY the un-sent rectangles on the grid, so the retry is pressing the
 ...
 * undone from the UI, because rows are immutable by design, so the only route back is
 ...
 * space above each heading than below, so the grouping is carried by proximity
